...
4. Add a custom `session_id` to the message's `data` object.  The custom `session_id` value starts a new chat session between your client and the LLMControls server, and can be useful in keeping conversations and AI context separate.
