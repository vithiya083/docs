...
To use this component in a flow, connect a component that outputs [DataFrame](https://devrel.mintlify.app/Concepts/LLMCobjects#dataframe-object%E2%80%8B) to the **DataFrame Operations** component.
...
This component performs operations on [Data](https://devrel.mintlify.app/Concepts/LLMCobjects#data-object%E2%80%8B) objects, including selecting keys, evaluating literals, combining data, filtering values, appending/updating data, removing keys, and renaming keys.

1. To use this component in a flow, connect a component that outputs [Data](https://devrel.mintlify.app/Concepts/LLMCobjects#data-object%E2%80%8B) to the **Data Operations** component's input. All operations in the component require at least one [Data](https://devrel.mintlify.app/Concepts/LLMCobjects#data-object%E2%80%8B) input.
...
The component supports the following operations. All operations in the **Data operations** component require at least one [Data](https://devrel.mintlify.app/Concepts/LLMCobjects#data-object%E2%80%8B) input.
...
This component converts one or multiple [Data](https://devrel.mintlify.app/Concepts/LLMCobjects#data-object%E2%80%8B) objects into a [DataFrame](https://devrel.mintlify.app/Concepts/LLMCobjects#dataframe-object%E2%80%8B). Each Data object corresponds to one row in the resulting DataFrame. Fields from the `.data` attribute become columns, and the `.text` field (if present) is placed in a 'text' column.

1. To use this component in a flow, connect a component that outputs [Data](https://devrel.mintlify.app/Concepts/LLMCobjects#data-object%E2%80%8B) to the **Data to Dataframe** component's input. This example connects a **Webhook** component to convert `text` and `data` into a DataFrame.
2. To view the flow's output, connect a **Chat Output** component to the **Data to Dataframe** component.

![4 Pn](/images/4.png)

3. Send a POST request to the **Webhook** containing your JSON data. Replace `YOUR_FLOW_ID` with your flow ID. This example uses the default LLMC server address.
...
This component converts [Message](https://devrel.mintlify.app/Concepts/LLMCobjects#message-object%E2%80%8B) objects to [Data](https://devrel.mintlify.app/Concepts/LLMCobjects#data-object%E2%80%8B) objects.
...
4. In the **File Path** field, enter the path for your saved file. This example uses `./output/employees.xlsx` and `./output/employees.md` to save the files in a directory relative to where LLMC is running. The component accepts both relative and absolute paths, and creates any necessary directories if they don't exist.
...
5. Send a POST request to the **Webhook** containing your JSON data. Replace `YOUR_FLOW_ID` with your flow ID. This example uses the default LLMC server address.
...
To use the **Smart function** component, you must connect it to a [Language Model](https://devrel.mintlify.app/Components/Models#language-model) component, which the component uses to generate a function based on the natural language instructions in the **Instructions** field.
...
<Note>
  **important**
  ...
  This component is in **Legacy**, which means it is available for use but no longer in active development. Instead, use the [<u>Data operations</u>](https://devrel.mintlify.app/Components/Processing#data-operations) component.
</Note>

This component modifies metadata of input objects. It can add new metadata, update existing metadata, and remove specified metadata fields. The component works with both [Message](https://devrel.mintlify.app/Concepts/LLMCobjects#message-object%E2%80%8B) and [Data](/Concepts/LLMCobjects) objects, and can also create a new Data object from user-provided text.
...
<Accordion title="Parameters">
  **Inputs**
  ...
  | text_in       | User Text        | Text input; the value is contained in the 'text' attribute of the [<u>Data</u>](https://docs.langflow.org/concepts-objects#data-object) object. Empty text entries are ignored. |
...
<Note>
  **important**
  ...
  This component is in **Legacy**, which means it is available for use but no longer in active development. Prior to LLMC version 1.1.3, this component was named **Merge Data**.
</Note>

This component combines multiple data sources into a single unified [Data](/Concepts/LLMCobjects) object.
...
<Accordion title="Parameters">
  ...
  **Outputs**
  ...
  | **Name**    | **Display Name** | **Info**                                                                                                                                               |
  | :---------- | :--------------- | :----------------------------------------------------------------------------------------------------------------------------------------------------- |
  | merged_data | Merged Data      | A single [<u>Data</u>](https://docs.langflow.org/concepts-objects#data-object) object containing the combined information from all input data objects. |
</Accordion>
...
1. To use this component in a flow, connect two components that output [Messages](https://devrel.mintlify.app/Concepts/LLMCobjects#message-object%E2%80%8B) to the **Combine Text** component's **First Text** and **Second Text** inputs. This example uses two **Text Input** components.
...
This component dynamically creates a [Data](/Concepts/LLMCobjects) object with a specified number of fields.
...
<Accordion title="Parameters">
  ...
  **Outputs**
  ...
  | **Name** | **Display Name** | **Info**                                                                                                                       |
  | :------- | :--------------- | :----------------------------------------------------------------------------------------------------------------------------- |
  | data     | Data             | A [<u>Data</u>](https://docs.langflow.org/concepts-objects#data-object) object created with the specified fields and text key. |
</Accordion>
...
This component filters a [Data](/Concepts/LLMCobjects) object based on a list of keys.
...
<Note>
  **important**
  ...
  This component is in **Legacy**, which means it is available for use but no longer in active development. Instead, use the [<u>Parser</u>](https://devrel.mintlify.app/Components/Processing#parser) component.
</Note>
...
<Accordion title="Parameters">
  **Inputs**
  ...
  | **Name**    | **Display Name** | **Info**                                                                                                                                                                      |
  | :---------- | :--------------- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
  | input_value | Input            | Data object to filter ([<u>Message</u>](https://docs.langflow.org/concepts-objects#message-object) or [<u>Data</u>](https://docs.langflow.org/concepts-objects#data-object)). |
  ...
  **Outputs**
  ...
  | **Name**      | **Display Name** | **Info**                                                                                                |
  | :------------ | :--------------- | :------------------------------------------------------------------------------------------------------ |
  | filtered_data | Filtered Data    | Filtered data as list of [<u>Data</u>](https://docs.langflow.org/concepts-objects#data-object) objects. |
</Accordion>
...
This component selects a single [Data](/Concepts/LLMCobjects) item from a list.
...
<Accordion title="Parameters">
  ...
  **Outputs**
  ...
  | **Name**      | **Display Name** | **Info**                                                                                   |
  | :------------ | :--------------- | :----------------------------------------------------------------------------------------- |
  | selected_data | Selected Data    | The selected [<u>Data</u>](https://docs.langflow.org/concepts-objects#data-object) object. |
</Accordion>