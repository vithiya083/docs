...
| Is Metadata    | When loading data using LangChain or LLMC, additional attributes may be stored under a metadata object. If the target attribute is stored this way, enable this option. It adjusts the query by generating a filter in the format: `{"metadata.<attribute_name>": "<value>"}`                                                                                                                     |
...
The **MCP connection** component exposes Model Context Protocol (MCP) servers, including your other flows, as tools for Langflow agents. For information about this component, see [Use LLMC as an MCP client](https://docs.langflow.org/mcp-client).

### **MCP Tools (deprecated)**

The **MCP Tools (stdio)** and **MCP Tools (SSE)** components are deprecated as of LLMC version 1.3. They are replaced by the [MCP connection component](https://docs.langflow.org/mcp-client).
