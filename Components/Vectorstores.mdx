---
title: "Vector store components in LLMControls"
---
...
Alternatively, connect the vector database component's **Retriever** port to a [retriever tool](https://devrel.mintlify.app/Components/Tools#retriever-tool), and then to an [agent](/Components/Agents) component. This enables the agent to use your vector database as a tool and make decisions based on the available data.
...
**Flexible Setup** \
While you focus on your content, LLMControls is smartly configured to perform at its best, adapting to different needs and environments. Developers can still fine-tune things like speed, search preferences, and more if needed.

**Seamless Content Capture**\
Just upload a document or paste a link, and LLMControls takes care of the rest. It understands and organizes the content instantly, so you can find what you need later in just a few words.
...
1. **Embedding Model**: Use your own embedding model by connecting an [Embeddings](/Components/Embeddings) component in LLMControls.
...
8. To view the [DataFrame](https://devrel.mintlify.app/Concepts/LLMCobjects#dataframe-object%E2%80%8B) generated from the **OpenAI** component's response, in the **Structured Output** component, click **Inspect output**. The DataFrame is passed to a **Parser** component, which parses the contents of the **Keywords** column into a string.
...
3. Optionally, to persist the Chroma database, in the **Persist** field, enter a directory to store the `chroma.sqlite3` file. This example uses `./chroma-db` to create a directory relative to where LLMControls is running.
...
<Accordion title="Parameters">
  **Inputs**
  ...
  | **Name**                         | **Type**   | **Description**                                                                   |
  | :------------------------------- | :--------- | :-------------------------------------------------------------------------------- |
  | collection_name                  | String     | The name of the Chroma collection. Default: "LLMControls".                        |
...
The **Local DB** component is LLMControls' enhanced version of Chroma DB.
...
The **Ingest** mode works similarly to [ChromaDB](https://devrel.mintlify.app/Components/Vectorstores#chroma-db), and persists your database to the LLMControls cache directory. The LLMControls cache directory location is specified in `LLMC_CONFIG_DIR`. For more information .
...
<Accordion title="Parameters">
  **Inputs**
  ...
  | persist_directory               | String     | Path to save the FAISS index. It is relative to where LLMControls is running.    |
  | search_query                    | String     | The query to search for in the vector store.                                     |
  | ingest_data                     | Data       | The list of data to ingest into the vector store.                                |
