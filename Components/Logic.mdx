---
title: "Logic components in LLMControls"
---
...
This flow creates a summarizing "for each" loop with the [Loop](https://devrel.mintlify.app/Components/Logic#loop%E2%80%8B) component.

The component iterates over a list of [Data](https://devrel.mintlify.app/Concepts/LLMCobjects#data-object%E2%80%8B) objects until it's completed, and then the **Done** loop aggregates the results.
...
When the **Loop** component runs out of `Data`, the **Done** loop activates, which counts the number of pages and summarizes their tone with another **Prompt**. This is represented in LLMControls by connecting the Parser component's **Data List** output to the Loop component's `Data` loop input.
...
The operator looks for single strings based on your defined [operator behavior](https://devrel.mintlify.app/Components/Logic#operator-behavior%E2%80%8B), but it can also search for multiple words by regex matching.
...
- In the **Operator** field, enter `regex`. The component looks for the strings `urgent`, `warning`, and `caution`. For more operators, see [Operator behavior](https://devrel.mintlify.app/Components/Logic#operator-behavior%E2%80%8B).
...
This component iterates over a list of [Data](https://devrel.mintlify.app/Concepts/LLMCobjects#data-object%E2%80%8B) objects, outputting one item at a time and aggregating results from loop inputs.
...
This component allows you to run any flow stored in your LLMControls database without opening the flow editor.
...
<Note>
  **important**
  ...
  This component is in **Legacy**, which means it is no longer in active development as of LLMControls version 1.3.
</Note>
...
<Note>
  **important**
  ...
  This component is deprecated as of LLMControls version 1.1.2. Instead, use the [<u>Run flow component</u>](https://devrel.mintlify.app/Components/Logic#run-flow%E2%80%8B)
</Note>
