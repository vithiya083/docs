---
title: "Helper components in LLMC"
---
...
Chat memory in LLMC is stored either in local LLMC tables with `LCBufferMemory`, or connected to an external database.
...
The **Batch Run** component runs a language model over **each row** of a [DataFrame](https://docs.langflow.org/concepts-objects#https://devrel.mintlify.app/Concepts/LLMCobjects#dataframe-object%E2%80%8Bdataframe-object) text column and returns a new DataFrame with the original text and an LLM response.
...
The Current Date component returns the current date and time in a selected timezone. This component provides a flexible way to obtain timezone-specific date and time information within a LLMC pipeline.
...
<Note>
  **info**
  ...
  Prior to LLMC 1.1, this component was known as the Chat Memory component.
</Note>
...
For more information on configuring memory in LLMC.
...
<Accordion title="Parameters">
  **Inputs**
  ...
  | **Name**    | **Type** | **Description**                                                                                                                 |
  | :---------- | :------- | :------------------------------------------------------------------------------------------------------------------------------ |
  | memory      | Memory   | Retrieve messages from an external memory. If empty, the LLMC tables are used.                                                  |
...
This component stores chat messages or text in LLMC tables or external memory.

In this example, the **Message Store** component stores the complete chat history in a local LLMC table, which the **Message History** component retrieves as context for the LLM to answer each question.
...
<Accordion title="Parameters">
  **Inputs**
  ...
  | memory      | Memory   | The external memory to store the message. If empty, the LLMC tables are used.                      |
