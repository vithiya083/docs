...
LLM Controls is an application development platform, but it's also a runtime you can call through an API with Python, JavaScript, or HTTP.
...
3. If you're using the cloud version, make sure to create an API key in your **Settings \> LLM Controls API** tab and set it as an environment variable (LLMC_API_KEY) in your development environment before making requests.
...
Polling is added to the `/run` endpoint's `payload`. To assist with formatting, you can define polling in the LLM Controls MCP **Input Schema** pane before copying the code snippet.
...
3. For example, to change the LLM Controls provider from OpenAI to Groq, and include your Groq API key with the request, select