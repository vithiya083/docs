...
1. In LLMControls, click **New Flow**, and then select the **Simple Agent** template.
   ...
   ![Simple Agent(2) Pn](/images/simple_agent\(2\).png)
...
1. To access API code for any flow in LLMControls, open the desired flow and click on the **"Publish"** button located at the top-right corner of the screen. A dropdown will appear with three options: **API access**, **Embed into site**, and **Shareable Playground**.
...
3. If you're using the cloud version, make sure to create an API key in your **Settings \> LLMControls API** tab and set it as an environment variable (LLMC_API_KEY) in your development environment before making requests.
...
In a production application, you probably want to select parts of this response to return to the user, store in logs, and so on. The next steps demonstrate how you can extract data from an LLMControls API response to use in your application.
...
You can include polling with your requests to temporarily modify flow parameters. Polling is added to the API request, and temporarily changes component parameters within your flow. Polling overrides the flow's components' settings for a single run only. They don't modify the underlying flow configuration or persist between runs.

Polling is added to the `/run` endpoint's `payload`. To assist with formatting, you can define polling in the LLMControls MCP **Input Schema** pane before copying the code snippet.
...
3. For example, to change the LLMControls provider from OpenAI to Groq, and include your Groq API key with the request, select