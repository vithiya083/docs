...
keywords: ["llm observability tools", "llm tracing and monitoring", "llm monitoring"]
---
...
<Frame>
  ![Screenshot From 2026 05 07 12 19 04 (1)](/images/Screenshot-from-2026-05-07-12-19-04-\(1\).png)
</Frame>
...
- Special handling for agent\_action events showing:
  - Tool name
  - Tool input
  - Log output
...
- Allows assessing the quality of a run’s output directly from any trace.
- Automatically detects the flow type and surfaces the relevant evaluation frameworks
- To use this,
  - Open any trace and click the Evaluate tab.
  - Select the metrics you want to run.
  - Provide a reference answer or expected output if required to unlock accuracy metrics.
  - Click Run
    <Accordion title="Available Metrics">
      | **LLM Judge** | General text flows | Safety, correctness, coherence, helpfulness,<br /> conciseness, groundedness |
      | :-- | :-- | :-- |
      | **RAGAS** | RAG flows | Faithfulness, answer relevancy, context precision, context recall |
      | **DeepEval** | General text flows | Hallucination, bias, toxicity |
      | **Extraction** | JSON output flows | Field accuracy, field presence, schema match |
      | **Transcription** | Transcription flows | WER, ROUGE-1, ROUGE-2, ROUGE-L |
      | **Classification** | Label as output flows | Exact match, normalized match |
    </Accordion>
...
- Type: Span type (CHAT\_MODEL, TOOL, CHAIN, etc.)
...
If you're building a custom component and want it to appear in LLMC traces, wrap your component execution with **mlflow\_component\_span:**

**from llmc.utils.mlflow\_tracing import mlflow\_component\_span**
...
- **span.set\_inputs(inputs)**: Log component inputs (dict)
- **span.set\_outputs(outputs)**: Log component outputs (dict)
