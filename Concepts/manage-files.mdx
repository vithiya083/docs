...
The **File management** system is available at the `/files` URL. For example, if you're running LLMC at the default `http://localhost:7860` address, the **File management** system is located at `http://localhost:7860/files`.

To upload a file from your local machine:

1. From the **My Files** window at `http://localhost:7860/files`, click **Upload**.
2. Select the file to upload. The file is uploaded to LLMC.
...
1. Include the [File](https://docs.langflow.org/components-data#file) component in a flow.
...
For an example of using the **File** component in a flow, see the [Document QA tutorial project](https://docs.langflow.org/document-qa).

<Note>
  NOTE

  If you prefer a one-time upload, the [<u>File</u>](https://docs.langflow.org/components-data#file) component still allows one-time uploads directly from your local machine.
</Note>
