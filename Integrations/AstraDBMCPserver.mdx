---
title: "Connect an Astra DB MCP server to LLMC"
---

Use the [MCP connection component](https://docs.langflow.org/mcp-client) to connect LLMC to a [Datastax Astra DB MCP server](https://github.com/datastax/astra-db-mcp).
...
5. Create a [Simple agent starter project](https://docs.langflow.org/simple-agent) if you want to follow along with this guide. Otherwise, you can use an existing flow or create a new, blank flow.
...
<Note>
  **important**
  ...
  LLMC passes environment variables from the `.env` file to MCP, but not global variables declared in the UI. To add the values for `ASTRA_DB_APPLICATION_TOKEN` and `ASTRA_DB_API_ENDPOINT` as global variables, add them to LLMC's `.env` file at startup. For more information, see [<u>global variables</u>](https://docs.langflow.org/configuration-global-variables).
</Note>
...
Since LLMC is connected to your Astra DB database through the MCP, the agent chooses the correct tool and connects to your database to retrieve the answer.